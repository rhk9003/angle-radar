...
之後在 app 裡用該碼登入 → 側邊欄顯示「剩餘次數 20」→ 每出一份切角報告扣 1；沒有交付可用報告時會自動退還。

第一次 `consume` 或 `refund` 時，程式會自動建立 `quota_transactions` 分頁：

| timestamp | request_id | code | action | delta | remaining |
|---|---|---|---|---:|---:|

同一個 `request_id` 的 `consume` 與 `refund` 各最多生效一次。重送請求只回傳
目前餘額，不會重複扣除或重複加回。更新 `Code.gs` 後必須在 Apps Script
「管理部署作業」建立新版本，現有 `/exec` 網址才會套用新邏輯。
...
使用者在結果卡上的後續行為會自動寫入 `usage_events` 分頁：

| `event_type` | 代表行為 |
|---|---|
| `favorite_added` | 收藏一張切角卡 |
| `favorite_removed` | 取消收藏 |
| `prompt_copied` | 成功複製該卡 Prompt |
| `favorites_downloaded` | 下載當下收藏的卡片、Prompt 與引用 |

`request_id` 可與 `usage_logs` 的同名欄位連接；`angle_key`、`angle_index`、`angle_name` 用來辨識卡片。下載事件的 `details` 會保存當下收藏數量與卡片清單。`usage_events` 也會在第一個事件發生時自動建立，不需要手動新增。

## 運作邏輯

- 登入：`check`（不扣）→ 顯示剩餘
- 出報告：`consume`（以 `request_id` 冪等地原子扣 1；remaining>0 才扣得動）
- 跑失敗：`refund`（只有同一 request 已 consume 才能退，且最多退一次）
- 切角回饋：`feedback`（寫入 `feedback` 分頁，不扣次數）
- 使用紀錄：`log_usage`（寫入 `usage_logs` 分頁，不扣次數；管理者分析也會記錄）
- 最終行為：`track_event`（寫入 `usage_events` 分頁，記錄收藏、複製 Prompt 與下載）
